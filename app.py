...
if mintsoft_file and dsd_file:
    ...
    try:
        ...
        # Deduplicate DSD on address so each address maps to exactly one Sequence
        dsd_df = dsd_df.drop_duplicates(subset=["Address Line One Clean"])

        # -----------------------------
        # Merge - Add Sequence to mintsoft_df
        # -----------------------------
        mintsoft_df = mintsoft_df.merge(
        ...
        mintsoft_df["Sequence"] = mintsoft_df["Sequence"].fillna("MISSING")

        # -----------------------------
        # Pick Sheet
        # -----------------------------
        pick_df = mintsoft_df.sort_values(
            by=[
                "Product Name",
                "Courier Service",
                "Sequence"
        ...
        labels_df = labels_df.sort_values(
            by=["Courier Service", "Sequence"]
        )

        # -----------------------------
        # Milk Sheet
        # -----------------------------
        milk_products_df = mintsoft_df[
            mintsoft_df["Product Name"]
            .astype(str)
